...
app.whenReady().then(() => {
    ...
    ipcMain.handle('save-pdf', async (_event, { filename, studentName, siblingNames, pdfBuffer, customPath }) => {
        const baseDir = customPath;
        if (!baseDir) {
            throw new Error('Invoice output folder not set for this branch');
        }
        const sanitize = (s) => s.replace(/[\\/:*?"<>|]/g, '').trim();
        const nameSet = new Set([studentName, ...siblingNames].map(sanitize).filter(Boolean));
        let targetDir = null;
